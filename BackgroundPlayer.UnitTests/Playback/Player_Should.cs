...
namespace BackgroundPlayer.UnitTests.Playback
{
	public class Player_Should
	{
		...
		[Theory]
		[AutoMoqData]
		public async Task ShowLastImageWhenTheresNoNextImage([Frozen] Mock<IWindowsBackground> windowsBackgroundMock, Player player, Skin skin, CancellationTokenSource cancellationTokenSource)
		{
			await player.PlaySkin(skin, cancellationTokenSource.Token);

			windowsBackgroundMock.Verify(x => x.Refresh(It.Is<string>(s => s == skin.Images.Last())), Times.Once);
		}
